...
Program EjercicioEntregableChequeo;
...
Procedure ImprimirVector(v:TProductos);
...
//a) Implemente un módulo que recibe la estructura de datos de productos y un código de producto y retorne el nombre de este. En caso de que el código no se encuentre debe retornar un string vacío. Implemente una búsqueda binaria/dicotómica. 
...
//c) Generar una lista enlazada de ventas ordenada por nombre de cliente junto con los productos que compró. Los datos de cada cliente se ingresan desde el teclado. Primero se ingresa el nombre, luego la cantidad de productos comprados (máximo 10) y luego la información de cada producto. La carga finaliza al ingresar “zzz” como nombre. En caso de ingresar una cantidad de productos mayor a 10 o un código de producto inválido debe volver a pedirse. 
...
Procedure Recorrer (Var L:Listaventas);

Var  pri,aux: Listaventas;
Begin
  aux := L;
  While (L <> Nil) Do
    Begin
...
Begin
  ...
  ImprimirVector(VectorCatalogo);
  Writeln('Ingrese el codigo a buscar  ');
  Readln(codigo);
  BusquedaDicotomica(VectorCatalogo,codigo,nombre,dimL);
  Writeln('El nombre del codigo  ',  codigo  , 'es: ',  nombre);
  //-------PuntoAResuelto-------
  Writeln('Ingrese el codigo a buscar si existe en el vector ');
  Readln(codigo);
  If (existe(VectorCatalogo,codigo,dimL))Then
    Writeln('Existe ')
  Else
    Writeln('No Existe ');
  //-------PuntoBResuelto-------
